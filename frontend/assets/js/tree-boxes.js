...
function treeBoxes(urlService, jsonData)
{
	...
	// 1) ******************* Update the nodes *******************
		...
		.text(function(d) {
			return 'Filtering: ' + d.filtering;
		})
		.append("tspan") // Ajoutez un autre tspan pour la coordination
		.attr('x', rectNode.width / 2 + tooltip.textMargin)
		.attr('dy', '1.2em') // Déplacez vers le bas d'une ligne
		.text(function(d) {
			return 'Coordination: ' + d.coordination;
		})
		// Continuez de la même manière pour les autres propriétés...
		.append("tspan")
		.attr('x', rectNode.width / 2 + tooltip.textMargin)
		.attr('dy', '1.2em') // Déplacez vers le bas d'une ligne
		.text(function(d) {
			return 'Antispoofing: ' + d.antispoofing;
		})
		.append("tspan")
		.attr('x', rectNode.width / 2 + tooltip.textMargin)
		.attr('dy', '1.2em') // Déplacez vers le bas d'une ligne
